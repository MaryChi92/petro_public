...
class Course(models.Model):
    ...
    lessons_starts_on = models.DateTimeField(verbose_name="Дата старта уроков", blank=True, null=True)
    open = models.CharField(verbose_name="Откроется в", max_length=50, blank=True, null=True)
    group_is_full = models.BooleanField(verbose_name="Поток набран", default=False)
    ...
    class Meta:
        ...
        ordering = ["-on_sale", "title"]

    def get_final_price(self, discount):
        final_price = int(self.price - self.price * discount / 100)
        return final_price
    ...
    def get_free_lesson_id(self):
        free_lesson = self.lessons.filter(is_free=True).first()
        if free_lesson:
            return free_lesson.id
        else:
            return False
...
class Lesson(models.Model):
    class LessonType(models.TextChoices):
        ONLINE_LESSON = ("Онлайн-урок", "Онлайн-урок")
        VIDEO_LESSON = ("Видеоурок", "Видеоурок")
    ...
    image_free = models.ImageField(verbose_name="Изображение к карточке бесплатного урока", blank=True, null=True, upload_to="free_lessons_img/")
    description = models.TextField(verbose_name="Описание урока", blank=True)
    description_homework = models.TextField(verbose_name="Описание домашнего задания к уроку", blank=True)
    video_file = models.CharField(verbose_name="Видео в записи", max_length=200, blank=True)
    ...
    certificate = models.ImageField(verbose_name="Сертификат", blank=True, null=True, upload_to="lessons_certificates/")
    created_at = models.DateTimeField(verbose_name='Создан', auto_now_add=True)
    updated_at = models.DateTimeField(verbose_name='Обновлен', auto_now=True)
    deleted = models.BooleanField(verbose_name="Удален", default=False)
    ...
    @staticmethod
    def get_all_free_lessons():
        free_lessons = Lesson.objects.filter(is_free=True)
        return free_lessons
...
class StudentCourse(models.Model):
    ...
    lessons_starts_on = models.DateTimeField(verbose_name="Дата старта потока", blank=True, null=True)
    ...
    order_number = models.CharField(verbose_name="Номер заказа", max_length=8, unique=True)
    purchase_price = models.IntegerField(verbose_name="Цена покупки", blank=True, null=True)
    promocode = models.CharField(verbose_name="Примененный промокод", max_length=50, blank=True)
    ...
    def create_order_number(self):
        ...
        order_number = ''.join(secrets.choice(letters_and_digits) for i in range(8))
        self.order_number = order_number
        self.save()

    def get_completed_lessons_quantity(self):
        return self.lesson_number - 1

    @staticmethod
    def get_number_of_active_and_completed_courses(user):
        active_courses_number = 0
        completed_courses_number = 0
        student_courses = StudentCourse.objects.filter(user=user)
        for student_course in student_courses:
            if student_course.is_completed:
                completed_courses_number += 1
            else:
                active_courses_number += 1

        active_and_completed_courses = (active_courses_number, completed_courses_number)

        return active_and_completed_courses

    def has_students_group_started(self):
        if not self.lessons_starts_on or self.lessons_starts_on <= now():
            return True
        else:
            return False
